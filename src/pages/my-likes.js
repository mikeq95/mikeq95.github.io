...
function MyLikesInner() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error) console.error('Failed to load likes:', error);
        setPosts(data ?? []);
        setFetching(false);
      });
  }, [user]);
  ...
  if (!user) {
    return (
      <div className={styles.emptyState}>
        <Icon icon="mdi:heart-off-outline" width={96} height={96} className={styles.emptyIcon} />
        <p className={styles.emptyText}>{isEn ? 'Please log in to see your liked posts.' : '请先登录查看你的点赞记录。'}</p>
  ...
  if (posts.length === 0) {
    return (
      <div className={styles.emptyState}>
        <Icon icon="mdi:heart-off-outline" width={96} height={96} className={styles.emptyIcon} />
        <p className={styles.emptyText}>{isEn ? 'No liked posts yet.' : '还没有点赞任何文章。'}</p>
  ...
  return (
    <Layout title={isEn ? 'My Likes' : '我的点赞'} description={isEn ? 'Posts you liked' : '你点赞过的文章'}>
      <main className={styles.container}>
        <h1 className={styles.pageTitle}>{isEn ? 'My Likes ❤️' : '我的点赞 ❤️'}</h1>
