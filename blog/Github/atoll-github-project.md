...
date: 2026-08-21
tags: [macos, swift, open-source, notch, media]
description: Atoll 是一个开源 macOS 应用，把 MacBook 的刘海屏改造成媒体控制、系统监控和生产力工具的集合面板。本文基于源码阅读整理。
---
MacBook 的刘海屏大多数时候就是个摆设——要么显示摄像头指示灯，要么啥都没有。Atoll 把这块空间重新利用了一下，把它变成一个可以展开的控制面板。
{/* truncate */}
---
## 介绍
...
---
## AI-friendly
把下面这段给 Claude Code 或者 Codex，它可以独立把 Atoll 装起来：
```
Install Atoll on macOS from source:
1. git clone https://github.com/Ebullioscopic/Atoll.git && cd Atoll
2. Open DynamicIsland.xcodeproj in Xcode 15+
3. Set the signing team to your Apple Developer account (or Personal Team)
4. Build and run the DynamicIsland scheme on a MacBook with a notch (14/16-inch MBP, macOS 14+)
5. On first launch, grant Accessibility, Screen Recording, Calendar, and Music permissions in System Settings → Privacy & Security, then fully quit and relaunch
Requirements: macOS 14+, Xcode 15+, MacBook with physical notch
```
---
## 卸载和下次运行
