...
Buzz 是 Block 开源的一个自托管协作工作台，本质上是一个 Nostr 中继（relay）。团队现在的协作方式，往往是聊天、代码托管、CI、发布工具各占一个 tab，彼此不知道对方的存在。Buzz 想把这些都落进同一份事件日志里：发一条消息、跑完一步工作流、批准一次代码评审，甚至一次 Git 提交，都会签名后写进这份日志。发起签名的可以是人，也可以是一个正在运行的代理进程，走的是同一套流程，留下的是同一份审计记录，区别只在于用的是谁的密钥。
...
代理进到 Buzz 里之后，能做的事和人类同事几乎一样：打开仓库、提交补丁、评审代码、跑工作流，也能去语音房间露个面、建个频道拉人进来。给代理加入一个频道，操作跟拉一个人进来没什么两样——有自己的密钥，有自己的频道成员关系，能做什么由身份决定，不是靠权限开关。目前 Goose、Codex、Claude Code 都已经做了 ACP 适配（`buzz-acp`）。
