...
Discord 官方客户端能改的地方不多，消息历史看不到编辑前的版本，主题也只有官方给的那几套。[Vencord](https://github.com/Vendicated/Vencord) 就是干这个的——给 Discord 客户端塞一套插件系统，装完之后原来的界面还是那个界面，但设置里多出一个面板，能装插件、换主题、改行为。

{/* truncate */}

> 这篇文章没有在真实 Discord 客户端上执行注入验证：本机装的 Discord 是日常在用的真实账号，改动它有风险，所以只核实了安装脚本本身的内容和官方文档描述，没有实际跑一遍安装流程。下文会说明具体验证到了哪一步。
