...
最近终端里跑代码代理（Claude Code、Codex 之类）越来越常见，但大部分终端本质上还是给敲命令用的，不太关心"代理在后台干什么"这件事。刷到 [Otty](https://otty.sh) 这个新工具，官网第一句话就是"为在意每次按键手感的人设计"，同时明确提到是为已经在用的代码代理调优过的，用下来觉得值得记一篇。
...
Otty 是一款原生、GPU 加速的跨平台终端模拟器，macOS 上用 Metal 渲染，Windows 上用 DirectX。定位介于"传统终端"和"完整的 Agent 开发环境"之间——既保留了终端本身简洁、键盘优先的操作方式，又针对终端里跑代理会话这件事做了专门设计。

开发方是 appmakes.io，目前不是开源项目，官网提供的是编译好的应用下载。
